#! /bin/bash

if [[ $1 == "test" ]]
then
  PSQL="psql --username=postgres --dbname=worldcuptest -t --no-align -c"
else
  PSQL="psql --username=freecodecamp --dbname=worldcup -t --no-align -c"
fi

# Do not change code above this line. Use the PSQL variable above to query your database.

echo "$($PSQL "TRUNCATE games,teams")"

cat games.csv | while IFS=',' read YEAR ROUND WINNER_TEAM OPPONENT_TEAM WINNER_GOALS OPPONENT_GOALS
do
  if [[ $YEAR != year ]]
  then
    echo $YEAR $ROUND $WINNER_TEAM $OPPONENT_TEAM $WINNER_GOALS $OPPONENT_GOALS
    # Check Winner exist
    WINNER_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER_TEAM'")
    # if not then ADD in the teams
    if [[ -z $WINNER_ID ]]
    then
      TEAM_INSERTED=$($PSQL "INSERT INTO teams(name) VALUES('$WINNER_TEAM')")
      # Get the id of the team
      #echo "$TEAM_INSERTED"
      if [[ $TEAM_INSERTED == "INSERT 0 1" ]]
      then
      #  echo "INSIDE THE TEAM INSERTED IF VALUES ARE: $TEAM_INSERTED $WINNER_TEAM"
        WINNER_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER_TEAM'")
      fi  
    fi
    # Check Opponent exist
    OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT_TEAM'")
    if [[ -z $OPPONENT_ID ]]
    then      
    # if not then ADD in the teams
      TEAM_INSERTED=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT_TEAM')")
    # Get the id of the team
      if [[ $TEAM_INSERTED == "INSERT 0 1" ]]
      then
        OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT_TEAM'")
      fi
    fi
    echo "Winner: $WINNER_ID , Opponent: $OPPONENT_ID "
    
    # ** games table insertions **
    GAME_INSERTED=$($PSQL "INSERT INTO games(year,round,winner_id,opponent_id,winner_goals,opponent_goals) VALUES ($YEAR,'$ROUND',$WINNER_ID,$OPPONENT_ID,$WINNER_GOALS,$OPPONENT_GOALS)")
    echo $GAME_INSERTED
  fi
done
